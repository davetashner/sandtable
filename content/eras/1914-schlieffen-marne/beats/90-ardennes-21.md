---
id: 1914:beat-ardennes-0821
title: 'Into the forest'
dateLabel: '21 August 1914 (chapter)'
from: 1914-08-21T00:00:00Z
to: 1914-08-22T00:00:00Z
branch: 1914:historical
focus: 1914:ardennes
links:
  battles: ['1914:ardennes']
  events: ['1914:ardennes-event-advance']
  people:
    [
      'person:joffre-joseph',
      'person:ruffey-pierre',
      'person:langle-de-cary-fernand-de',
      'person:albrecht-of-wurttemberg',
      'person:wilhelm-german-crown-prince',
    ]
  places: ['place:neufchateau', 'place:longwy', 'place:verdun']
sources:
  - source: source:afgg-1-1
  - source: source:doughty-2005
  - source: source:herwig-2009
  - source: source:zuber-2007
---

This is the main effort. While the 1st and 2nd Armies are being thrown out of Lorraine and Lanrezac is arguing his way north to the Sambre, Joffre sends the 3rd and 4th Armies north-east into the Ardennes — the blow Plan XVII was built for, into what he believes is the thin centre of the German line. His reasoning is not foolish: the reports of great German columns in Belgium must mean the centre has been stripped to pay for them.[^afgg-1-1] [^doughty-2005]

The centre has not been stripped. The German 4th and 5th Armies are marching south-west through the same forest at the same hour, on the same roads, for reasons of their own — they are the pivot the wheel turns on, and they are moving up to keep the line closed. Two pairs of armies are walking towards each other through eighty kilometres of wooded hills in which neither can see anything.[^herwig-2009] [^zuber-2007]

What ought to prevent the collision is reconnaissance, and it does not happen. The cavalry cannot see through the woods, the weather grounds the aircraft, and the orders that go out on the 21st tell corps commanders to attack whatever they find, immediately, without waiting to develop the situation — the doctrine of the _offensive à outrance_, which treats a halt to look around as the beginning of defeat. The French move in march column, guns behind infantry, into country where the road is the only place a gun can be brought into action.[^afgg-1-1] [^doughty-2005]
