...
npm run perf             # measure bundle, first map paint, frame rate, PMTiles cost; -- --live for the real bucket, -- --headed for a real GPU
                         ...
                         # The boot table's `blocked`/`worst` columns are sand-pmz.23's long tasks — about 6 s of
                         # shader compilation on the campaign, 0 on the map-less atlas; measured for --settle-ms past map-ready
